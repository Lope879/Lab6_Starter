...
function addRecipesToDocument(recipes) {
	...
	for (let entry in recipes) {
		...
		newRecipe.data = recipes[entry];
		mainReference.appendChild(newRecipe);
	}
}
...
function initFormHandler() {
	...
	let submitButton = document.querySelector('form button'); //B3
	submitButton.addEventListener('click', () => {
		const formData = new FormData(formReference);     //B4
		let recipeObject = {};
		for (let [key, value] of formData.entries()) {    //B5
			recipeObject[key] = value;
		}
		//console.log(recipeObject);
		let recipeEl = document.createElement('recipe-card'); //B6
		recipeEl.data = recipeObject;						  //B7
		const mainEl = document.querySelector('main');
		mainEl.appendChild(recipeEl);                         //B8
	...
	let clearButton = document.querySelector('button.danger');   //B10
